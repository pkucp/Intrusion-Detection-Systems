# -*- coding: utf-8 -*-
"""
Created on Fri Feb  7 08:49:08 2020
...
for row in [0,4,5]:
    testT[:,row] = np.log10(testT[:,row]+1)

#minX = np.min(trainX,0)
#maxX = np.max(trainX,0)

for row in range(len(minX)):
    testT[:,row] = (testT[:,row]-minX[row])/(maxX[row]-minX[row])
...
import os
for file in os.listdir("test/preprocess/dnn1layer_64_dr_0.01/"):
    model.load_weights("test/preprocess/dnn1layer_64_dr_0.01/"+file)
    y_train1 = y_test
...
model.load_weights("test/preprocess/dnn1layer_64_dr_0.01/"+name[score.index(max(score))])
